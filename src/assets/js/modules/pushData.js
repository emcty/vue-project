...
UserData.prototype = {
    ...
    getUserId: function () {
        this.userId = this.getPara('userId') || cookie('userId')
    },
    ...
    getTempUid: function () { //获取临时id(区分app和非app)
        ...
        if (this.app) {
            ...
            this.tempUid = this.getPara('tempSign')
        } else {
    ...
    getInfoFromJava: function () { //获取ip地址
        let _url = ConfigPath.ajaxurl + "/wap/platform/v1/ip",
            _this = this
        // $.ajax({
        //     url: _url + "?random=" + this.getTimeStamp(),
        //     type: "GET",
        //     dataType: "json",
    ...
    authtication: function () { //鉴权接口
        let _url = ConfigPath.dataAjaxUrl + "/report/access",
            _this = this,
            rids = this.rid
        // $.ajax({
        //     url: _url + "?appKey=" + ConfigPath.appkey + "&rid=" + rids + "&version=1.0",
        //     type: "post",
        //     dataType: "json",
        //     success: function (data) {
        //         if (data.status == 1) {
        ...
        //     }
        // })
        $request.post(_url + "?appKey=" + ConfigPath.appkey + "&rid=" + rids + "&version=1.0").then((data)=> {
            if (data.status == 1) {
                _this.ticData(data)
            } else {
                if (_this.errorFlag) {
    ...
    pushData: function (isHotTap) { //上报接口
        let _url = ConfigPath.dataAjaxUrl + "/report/js/upload",
            rids = this.rid,
            _this = this,
            uploadData = isHotTap ? {
                "url": this.url,
        ...
        // $.ajax({
        //     url: _url + "?rid=" + rids + "&version=1.0&ticket=" + this.ticket,
        //     type: "POST",
        //     dataType: "json",
        //     data: {
